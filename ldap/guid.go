...
func checkIsZero(guidBytes []byte) bool {
	for _, val := range guidBytes {
		if val != 0 {
			return false
		}
	}
...
func NewGuid(guidBytes []byte) (*Guid, error) {
	if len(guidBytes) != 16 {
		return nil, fmt.Errorf("guid length is unvalid")
	}
	return &Guid{
...
func (g *Guid) String() string {
	...
	if g.Data1 == 0 && g.Data2 == 0 && g.Data3 == 0 && checkIsZero(g.Data4[:]) {
		return ""
	}
	return fmt.Sprintf("%08x-%04x-%04x-%04x-%012x", g.Data1, g.Data2, g.Data3, g.Data4[:2], g.Data4[2:])
}
...
func (g *Guid) Alias() string {
	...
	if val, ok := ExtendedRights[guid]; ok {
		return fmt.Sprintf("%s(%s)", val, guid)
	}
	return guid
}
